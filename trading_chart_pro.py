"""
Trading Chart Pro v3 — Bodzilla IA Interactive
────────────────────────────────────────────────
Chat en sidebar que controla el chart en tiempo real.
Bodzilla IA puede dibujar soportes, resistencias, Fibonacci,
trendlines, agregar indicadores y responder preguntas.

Instalar:  pip install streamlit anthropic ccxt yfinance pandas numpy
...
ASSETS = {
    "🪙 Crypto":[
        {"label":"BTC/USDT","ccxt":"BTC/USDT","source":"ccxt"},
        {"label":"ETH/USDT","ccxt":"ETH/USDT","source":"ccxt"},
        {"label":"SOL/USDT","ccxt":"SOL/USDT","source":"ccxt"},
        {"label":"BNB/USDT","ccxt":"BNB/USDT","source":"ccxt"},
        {"label":"XRP/USDT","ccxt":"XRP/USDT","source":"ccxt"},
        {"label":"DOGE/USDT","ccxt":"DOGE/USDT","source":"ccxt"},
        {"label":"ADA/USDT","ccxt":"ADA/USDT","source":"ccxt"},
        {"label":"AVAX/USDT","ccxt":"AVAX/USDT","source":"ccxt"},
        {"label":"LINK/USDT","ccxt":"LINK/USDT","source":"ccxt"},
        {"label":"DOT/USDT","ccxt":"DOT/USDT","source":"ccxt"},
    ],
    "💱 Forex / Metales":[
        {"label":"XAUUSD","yf":"GC=F","source":"yfinance"},
        {"label":"XAGUSD","yf":"SI=F","source":"yfinance"},
        {"label":"EURUSD","yf":"EURUSD=X","source":"yfinance"},
        {"label":"GBPUSD","yf":"GBPUSD=X","source":"yfinance"},
        {"label":"USDJPY","yf":"USDJPY=X","source":"yfinance"},
        {"label":"AUDUSD","yf":"AUDUSD=X","source":"yfinance"},
    ],
    "📈 Stocks":[
        {"label":"SPY","yf":"SPY","source":"yfinance"},
        {"label":"QQQ","yf":"QQQ","source":"yfinance"},
        {"label":"AAPL","yf":"AAPL","source":"yfinance"},
        {"label":"TSLA","yf":"TSLA","source":"yfinance"},
        {"label":"NVDA","yf":"NVDA","source":"yfinance"},
        {"label":"MSFT","yf":"MSFT","source":"yfinance"},
        {"label":"GOOGL","yf":"GOOGL","source":"yfinance"},
        {"label":"AMZN","yf":"AMZN","source":"yfinance"},
    ],
    "🛢️ Commodities":[
        {"label":"Crude Oil","yf":"CL=F","source":"yfinance"},
        {"label":"Nat. Gas","yf":"NG=F","source":"yfinance"},
        {"label":"Copper","yf":"HG=F","source":"yfinance"},
        {"label":"Wheat","yf":"ZW=F","source":"yfinance"},
    ],
}
TIMEFRAMES = {
    "5m":{"ccxt":"5m","yf":"5m","period":"5d"},
    "15m":{"ccxt":"15m","yf":"15m","period":"5d"},
    "1h":{"ccxt":"1h","yf":"1h","period":"30d"},
    "4h":{"ccxt":"4h","yf":"4h","period":"60d"},
    "1D":{"ccxt":"1d","yf":"1d","period":"365d"},
    "1W":{"ccxt":"1w","yf":"1wk","period":"1000d"},
}
...
def fetch_ohlcv(label, source, ccxt_sym, yf_sym, tf):
    cfg = TIMEFRAMES[tf]
    try:
        if source == "ccxt":
            ex  = ccxt.binance({"enableRateLimit":True})
            raw = ex.fetch_ohlcv(ccxt_sym, cfg["ccxt"], limit=300)
            df  = pd.DataFrame(raw, columns=["timestamp","Open","High","Low","Close","Volume"])
            df["timestamp"] = pd.to_datetime(df["timestamp"], unit="ms", utc=True)
            df  = df.set_index("timestamp")
        else:
            t  = yf.Ticker(yf_sym)
            df = t.history(period=cfg["period"], interval=cfg["yf"])[["Open","High","Low","Close","Volume"]]
...
def calculate_indicators(df):
    ...
    ind["macd"]=macd_line.round(8).tolist(); ind["macd_signal"]=sig.round(8).tolist()
    ind["macd_hist"]=(macd_line-sig).round(8).tolist(); ind["macd_cur"]=round(float(macd_line.iloc[-1]),6)
    # BB
    ...
    # VWAP
    tp=(df["High"]+df["Low"]+df["Close"])/3; vwap=(tp*df["Volume"]).cumsum()/df["Volume"].cumsum()
    ...
    # Swing levels
    recent=df.tail(60)
    sh=recent["High"].rolling(5,center=True).max(); sl=recent["Low"].rolling(5,center=True).min()
    ...
    # Price
    ind["price"]=round(float(df["Close"].iloc[-1]),8); ind["change"]=round(float((df["Close"].iloc[-1]/df["Close"].iloc[-2]-1)*100),2)
    ind["high24"]=round(float(df["High"].tail(24).max()),8); ind["low24"]=round(float(df["Low"].tail(24).min()),8)
    return ind

# ── System Prompt ─────────────────────────────────────────────────────────────
SYSTEM_PROMPT = """Eres Bodzilla IA, analista experto en trading ICT/SMC integrado en una plataforma de charts interactiva.

Tienes DOS capacidades:
1. RESPONDER preguntas de trading, análisis, conceptos ICT/SMC, estrategias
2. EJECUTAR ACCIONES en el chart del usuario

Responde SIEMPRE con JSON válido con esta estructura exacta:
{"text": "Mensaje en español para el usuario (máx 5 líneas)", "actions": [...]}

ACCIONES DISPONIBLES:

draw_hline — línea horizontal en el chart
{"type":"draw_hline","price":1234.5,"color":"#00d084","label":"Soporte","style":"solid","width":1}

draw_fibonacci — niveles de Fibonacci automáticos
{"type":"draw_fibonacci","high":1300.0,"low":1200.0,"color":"#00aaff"}
...
{"type":"draw_trendline","p1_time":"2024-01-15T10:00:00","p1_price":1200.0,"p2_time":"2024-01-20T10:00:00","p2_price":1250.0,"color":"#ff7b00","label":"Tendencia alcista"}

highlight_zone — zona sombreada entre dos precios
{"type":"highlight_zone","price_top":1300.0,"price_bot":1280.0,"color":"rgba(0,208,132,0.15)","label":"OB Bullish"}

toggle_indicator — activar/desactivar indicador existente
{"type":"toggle_indicator","indicator":"ema10","visible":true}
Indicadores válidos: ema10, ema55, ema200, vwap, bb

add_ema — agregar EMA con periodo personalizado
{"type":"add_ema","period":21,"color":"#ffdd00"}

change_asset — cambiar activo del chart
{"type":"change_asset","symbol":"ETH/USDT"}

change_timeframe — cambiar timeframe
{"type":"change_timeframe","tf":"4h"}

clear_drawings — limpiar todas las líneas dibujadas por la IA
{"type":"clear_drawings"}

REGLAS IMPORTANTES:
- Usa SIEMPRE precios REALES del contexto del chart
- Para soportes/resistencias usa swing_highs y swing_lows del contexto
- Para Fibonacci usa fib_high y fib_low del contexto
- Colores del tema: verde=#00d084, azul=#00aaff, naranja=#ff7b00, rojo=#ef4444
- Si solo responden preguntas sin dibujar: actions=[]
- NUNCA menciones Claude ni Anthropic, eres Bodzilla IA
- Responde siempre en español
- El JSON debe ser válido y parseable"""

# ── Chat con Bodzilla IA ──────────────────────────────────────────────────────
def chat_with_bodzilla(api_key, messages, ind, df, asset_label, tf):
    ...
    recent = "".join(
        f"  {ts.strftime('%m-%d %H:%M')} O:{r['Open']:.6g} H:{r['High']:.6g} L:{r['Low']:.6g} C:{r['Close']:.6g}\n"
...
Fib High:{ind.get('fib_high',0)} | Fib Low:{ind.get('fib_low',0)} | Niveles:{ind.get('fib_levels',{})}
...
def build_chart_html(df, ind, label, tf, overlay_actions=None):
    if df.empty: return "<p style='color:#ef4444'>Sin datos</p>"

    candles = [{"time":int(ts.timestamp()),"open":round(float(r["Open"]),8),
                "high":round(float(r["High"]),8),"low":round(float(r["Low"]),8),
                "close":round(float(r["Close"]),8)} for ts,r in df.iterrows()]
    volumes = [{"time":int(ts.timestamp()),"value":round(float(r["Volume"]),4),
    ...
    def bl(key):
        ...
        return json.dumps([{"time":int(df.index[i].timestamp()),"value":v}
                           for i,v in enumerate(vals) if i<len(df) and v and not (isinstance(v,float) and np.isnan(v))])

    p=ind.get("price",0); chg=ind.get("change",0)
    cc="#00d084" if chg>=0 else "#ef4444"; cs2="+" if chg>=0 else ""
    ov_json=json.dumps(overlay_actions or [])
...
<div id="ctrl">
  ...
  <span style="color:#1e2d3d;margin:0 2px">|</span>
...
const OV_ACTIONS={ov_json};

const LS=LightweightCharts.LineStyle;
const ch=LightweightCharts.createChart(document.getElementById('cc'),{{
  width:document.getElementById('cc').offsetWidth,height:370,
...
const e10s=ch.addLineSeries({{color:'#00d084',lineWidth:1.5,title:'E10',visible:false}});e10s.setData(E10);
const e55s=ch.addLineSeries({{color:'#00aaff',lineWidth:1.5,title:'E55',visible:false}});e55s.setData(E55);
const e200s=ch.addLineSeries({{color:'#ff7b00',lineWidth:2,title:'E200',visible:false}});e200s.setData(E200);
const vws=ch.addLineSeries({{color:'#4a9eff',lineWidth:1,lineStyle:LS.Dashed,visible:false}});vws.setData(VW);
...
// Activar EMAs por defecto
e10s.applyOptions({{visible:true}});e55s.applyOptions({{visible:true}});e200s.applyOptions({{visible:true}});vws.applyOptions({{visible:true}});
...
const SEL_CLS={{'e10':'g','e55':'b','e200':'o','vwap':'b'}};
const BTN_IDS={{'e10':'be10','e55':'be55','e200':'be200','vwap':'bvwap'}};
...
OBB.forEach(o=>{{
  obL.push(cnd.createPriceLine({{price:o.top,color:'rgba(0,208,132,0)',lineWidth:1,lineStyle:0,axisLabelVisible:false}}));
  obL.push(cnd.createPriceLine({{price:o.bot,color:'rgba(0,208,132,0)',lineWidth:1,lineStyle:3,axisLabelVisible:false}}));
}});
OBR.forEach(o=>{{
  obL.push(cnd.createPriceLine({{price:o.top,color:'rgba(239,68,68,0)',lineWidth:1,lineStyle:0,axisLabelVisible:false}}));
  obL.push(cnd.createPriceLine({{price:o.bot,color:'rgba(239,68,68,0)',lineWidth:1,lineStyle:3,axisLabelVisible:false}}));
}});
FB.forEach(f=>{{
  fvgL.push(cnd.createPriceLine({{price:f.top,color:'rgba(0,170,255,0)',lineWidth:1,lineStyle:2,axisLabelVisible:false}}));
  fvgL.push(cnd.createPriceLine({{price:f.bot,color:'rgba(0,170,255,0)',lineWidth:1,lineStyle:3,axisLabelVisible:false}}));
}});
FR.forEach(f=>{{
  fvgL.push(cnd.createPriceLine({{price:f.top,color:'rgba(255,123,0,0)',lineWidth:1,lineStyle:2,axisLabelVisible:false}}));
  fvgL.push(cnd.createPriceLine({{price:f.bot,color:'rgba(255,123,0,0)',lineWidth:1,lineStyle:3,axisLabelVisible:false}}));
}});
...
function showLines(arr,on){{
  const opMap={{'rgba(0,208,132':on?0.8:0,'rgba(239,68,68':on?0.7:0,'rgba(0,170,255':on?0.6:0,'rgba(255,123,0':on?0.65:0,'rgba(167,139,250':on?0.5:0}};
  arr.forEach(l=>{{
    try{{
      const c=l.options().color;
      const key=Object.keys(opMap).find(k=>c.startsWith(k));
      if(key!=null){{
        const base=c.replace(/,[0-9.]+[)]$/,'');
        l.applyOptions({{color:base+','+opMap[key]+')'}});
      }}
    }}catch(e){{}}
  }});
}}
...
function initFib(){{
  const fibColors={{'0':'rgba(255,255,255,0.5)','0.236':'rgba(0,208,132,0.6)','0.382':'rgba(0,170,255,0.7)','0.5':'rgba(255,255,255,0.5)','0.618':'rgba(255,123,0,0.7)','0.786':'rgba(0,170,255,0.6)','1.0':'rgba(255,255,255,0.4)'}};
  Object.entries(FIBL).forEach(([k,v])=>{{
    fibL.push(cnd.createPriceLine({{price:v,color:fibColors[k]||'rgba(255,255,255,0.4)',lineWidth:1,lineStyle:2,axisLabelVisible:true,title:'Fib '+k}}));
  }});
  fibInit=true;
}}
function initTL(){{
  if(TLH.length>=2){{
    const pts=TLH.slice(-2);
    const s=ch.addLineSeries({{color:'rgba(239,68,68,0.5)',lineWidth:1,lineStyle:LS.Dashed,title:'Res TL'}});
    s.setData([{{time:pts[0][0],value:pts[0][1]}},{{time:pts[1][0],value:pts[1][1]}}]);
    tlSeries.push(s);
  }}
  if(TLL.length>=2){{
    const pts=TLL.slice(-2);
    const s=ch.addLineSeries({{color:'rgba(0,208,132,0.5)',lineWidth:1,lineStyle:LS.Dashed,title:'Sup TL'}});
    s.setData([{{time:pts[0][0],value:pts[0][1]}},{{time:pts[1][0],value:pts[1][1]}}]);
    tlSeries.push(s);
  }}
  tlInit=true;
...
function tog(k,cls){{
  const s=SM[k];const btn=document.getElementById(BTN_IDS[k]);
  const wasOn=btn.classList.contains(cls);
  if(wasOn){{btn.classList.remove(cls);s.applyOptions({{visible:false}});}}
  else{{btn.classList.add(cls);s.applyOptions({{visible:true}});}}
}}
function togBB(){{
  bbVis=!bbVis;const btn=document.getElementById('bbb');
  if(bbVis){{btn.classList.add('o');[bbus,bbls,bbms].forEach(s=>s.applyOptions({{visible:true}}));}}
  else{{btn.classList.remove('o');[bbus,bbls,bbms].forEach(s=>s.applyOptions({{visible:false}}));}}
}}
function togOvr(k,cls){{
  OVR[k]=!OVR[k];const btn=document.getElementById('b'+k);
  if(OVR[k])btn.classList.add(cls);else btn.classList.remove(cls);
  if(k==='ob')  showLines(obL,OVR[k]);
  if(k==='fvg') showLines(fvgL,OVR[k]);
  if(k==='liq') showLines(liqL,OVR[k]);
  if(k==='fib'){{if(!fibInit)initFib();showLines(fibL,OVR[k]);}}
  if(k==='tl'){{
    if(!tlInit)initTL();
    tlSeries.forEach(s=>s.applyOptions({{visible:OVR[k]}}));
  }}
}}
...
function clearDyn(){{
  dynLines.forEach(l=>{{try{{cnd.removePriceLine(l)}}catch(e){{}}}});
  dynSeries.forEach(s=>{{try{{ch.removeSeries(s)}}catch(e){{}}}});
  dynLines=[]; dynSeries=[];
}}
function applyActions(actions){{
  actions.forEach(a=>{{
    try{{
      if(a.type==='draw_hline'){{
        const ls=a.style==='dashed'?LS.Dashed:LS.Solid;
        dynLines.push(cnd.createPriceLine({{price:a.price,color:a.color||'#00d084',lineWidth:a.width||1,lineStyle:ls,axisLabelVisible:true,title:a.label||''}}));
      }}
      else if(a.type==='draw_fibonacci'){{
        const h=a.high,l=a.low;
        const fc={{'0':'rgba(255,255,255,0.5)','0.236':'rgba(0,208,132,0.7)','0.382':'rgba(0,170,255,0.8)','0.5':'rgba(255,255,255,0.6)','0.618':'rgba(255,123,0,0.8)','0.786':'rgba(0,170,255,0.7)','1.0':'rgba(255,255,255,0.5)'}};
        [0,0.236,0.382,0.5,0.618,0.786,1.0].forEach(f=>{{
          const price=h-(h-l)*f;
          dynLines.push(cnd.createPriceLine({{price,color:fc[String(f)]||a.color||'#00aaff',lineWidth:1,lineStyle:2,axisLabelVisible:true,title:'Fib '+f}}));
        }});
      }}
      else if(a.type==='draw_support_resistance'){{
        (a.supports||[]).forEach(p=>dynLines.push(cnd.createPriceLine({{price:p,color:'rgba(0,208,132,0.85)',lineWidth:1,lineStyle:0,axisLabelVisible:true,title:'Soporte'}})));
        (a.resistances||[]).forEach(p=>dynLines.push(cnd.createPriceLine({{price:p,color:'rgba(239,68,68,0.85)',lineWidth:1,lineStyle:0,axisLabelVisible:true,title:'Resistencia'}})));
      }}
      else if(a.type==='draw_trendline'){{
        const s=ch.addLineSeries({{color:a.color||'#ff7b00',lineWidth:1.5,lineStyle:LS.Dashed,title:a.label||'TL'}});
        const t1=a.p1_time?Math.floor(new Date(a.p1_time).getTime()/1000):0;
        const t2=a.p2_time?Math.floor(new Date(a.p2_time).getTime()/1000):0;
        if(t1&&t2)s.setData([{{time:t1,value:a.p1_price}},{{time:t2,value:a.p2_price}}]);
        dynSeries.push(s);
      }}
      else if(a.type==='highlight_zone'){{
        dynLines.push(cnd.createPriceLine({{price:a.price_top,color:a.color||'rgba(0,208,132,0.5)',lineWidth:1,lineStyle:2,axisLabelVisible:true,title:a.label||''}}));
        dynLines.push(cnd.createPriceLine({{price:a.price_bot,color:a.color||'rgba(0,208,132,0.3)',lineWidth:1,lineStyle:2,axisLabelVisible:false,title:''}}));
      }}
      else if(a.type==='add_ema'){{
        const closes=CA.map(c=>c.close);
        const k2=2/(a.period+1); let ev=[closes[0]];
        for(let i=1;i<closes.length;i++)ev.push(closes[i]*k2+ev[i-1]*(1-k2));
        const ns=ch.addLineSeries({{color:a.color||'#ffdd00',lineWidth:1.5,title:'EMA'+a.period}});
        ns.setData(CA.map((c,i)=>({{time:c.time,value:ev[i]}})));
        dynSeries.push(ns);
      }}
      else if(a.type==='toggle_indicator'){{
        const imap={{'ema10':e10s,'ema55':e55s,'ema200':e200s,'vwap':vws}};
        const cls_map={{'ema10':'g','ema55':'b','ema200':'o','vwap':'b'}};
        const btn_map={{'ema10':'be10','ema55':'be55','ema200':'be200','vwap':'bvwap'}};
        if(a.indicator==='bb'){{
          bbVis=a.visible;[bbus,bbls,bbms].forEach(s=>s.applyOptions({{visible:a.visible}}));
          const btn=document.getElementById('bbb');
          if(a.visible)btn.classList.add('o');else btn.classList.remove('o');
        }}else if(imap[a.indicator]){{
          imap[a.indicator].applyOptions({{visible:a.visible}});
          const btn=document.getElementById(btn_map[a.indicator]);
          if(a.visible)btn.classList.add(cls_map[a.indicator]);else btn.classList.remove(cls_map[a.indicator]);
        }}
      }}
      else if(a.type==='clear_drawings'){{clearDyn();}}
    }}catch(err){{console.warn('action err',a.type,err);}}
  }});
}}
if(OV_ACTIONS&&OV_ACTIONS.length)applyActions(OV_ACTIONS);

// RSI panel
const rc=LightweightCharts.createChart(document.getElementById('rc'),{{
  width:document.getElementById('rc').offsetWidth,height:65,
  layout:{{background:{{color:'#080c10'}},textColor:'#4a6278'}},
  grid:{{vertLines:{{color:'#0c1520'}},horzLines:{{color:'#0c1520'}}}},
...
window.addEventListener('resize',()=>{{
  ch.applyOptions({{width:document.getElementById('cc').offsetWidth}});
  rc.applyOptions({{width:document.getElementById('rc').offsetWidth}});
}});
...
def render_bubble(role, text):
    if role=="user":
        ...
        st.markdown(f"""<div style="display:flex;justify-content:flex-start;margin:3px 0;">
          <div style="background:{C['bg3']};border:1px solid {C['green']}33;border-left:2px solid {C['green']};
...
def render_action_badge(action):
    ...
    labels={"draw_hline":f"Línea @ {action.get('price','')}",
            "draw_fibonacci":f"Fibonacci {action.get('low','')}–{action.get('high','')}",
...
def get_auto_analysis(api_key, label, tf, ind_json, df_json):
    try:
        ind=json.loads(ind_json); df=pd.read_json(df_json)
        e10=round(ind["ema10"][-1],6) if ind.get("ema10") else "N/A"
        e55=round(ind["ema55"][-1],6) if ind.get("ema55") else "N/A"
        e200=round(ind["ema200"][-1],6) if ind.get("ema200") else "N/A"
        recent="".join(f"  {ts.strftime('%m-%d %H:%M')} C:{r['Close']:.6g}\n" for ts,r in df.tail(3).iterrows())
...
BSL:{ind.get('swing_highs',[])} SSL:{ind.get('swing_lows',[])}
Velas: {recent}
Responde con JSON: {{"text":"análisis bias+señal+niveles en formato ## BIAS / ## ESTRUCTURA / ## NIVELES CLAVE / ## SEÑAL / ## PLAN","actions":[]}}"""
        client=anthropic.Anthropic(api_key=api_key)
...
def main():
    ...
    with st.sidebar:
        # API key + TF
        api_key=st.text_input("API",value=load_api_key(),type="password",
                               placeholder="sk-ant-... API Key",label_visibility="collapsed")
        if api_key: save_api_key(api_key)
        st.session_state.timeframe=st.selectbox("TF",list(TIMEFRAMES.keys()),
        ...
        # Quick commands
        st.markdown(f'<div style="font-size:0.58rem;color:{C["muted"]};margin-bottom:3px;">Comandos rápidos</div>',unsafe_allow_html=True)
        QUICK=[
            ("📊 Análisis","Dame un análisis completo con bias, niveles clave y señal"),
            ("🎯 S/R","Traza los soportes y resistencias más importantes en el chart"),
            ("📐 Fibonacci","Dibuja los niveles de Fibonacci del último swing relevante"),
            ("📈 Trendlines","Traza las líneas de tendencia alcista y bajista"),
            ("⚡ LONG/SHORT","¿Hay setup de LONG o SHORT en este momento?"),
            ("🧹 Limpiar","Limpia todas las líneas que trazaste en el chart"),
        ]
    ...
    with st.spinner(f"Cargando {asset['label']}..."):
        df=fetch_ohlcv(asset["label"],asset["source"],asset.get("ccxt",""),asset.get("yf",""),tf)
    if df.empty:
        st.error(f"Sin datos para {asset['label']}."); return
    ...
    if do_send and msg_to_send and api_key:
        ...
        text,actions=chat_with_bodzilla(api_key,api_msgs,ind,df,asset["label"],tf)
    ...
    with tab_auto:
        if not api_key:
            st.markdown(f'<div style="background:{C["bg3"]};border:1px solid {C["orange"]};border-radius:4px;padding:8px 12px;font-size:0.76rem;color:{C["orange"]};">⚠ API Key requerida en sidebar</div>',unsafe_allow_html=True)
        else:
            ...
            with st.spinner("Analizando..."):
                ...
                text,_=get_auto_analysis(api_key,asset["label"],tf,ind_s,df.tail(20).to_json())
                secs=text.split("\n## ")
